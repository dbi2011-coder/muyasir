...
// ========================================================
// 🛡️ 1. جلب الطلاب مع تطبيق العزل التام للبيانات عبر (Supabase)
// ========================================================
async function loadStudentsForReports() {
    ...
    tableBody.innerHTML = '<tr><td colspan="8" style="text-align: center; padding: 20px;">جاري جلب بيانات الطلاب من السحابة... <i class="fas fa-spinner fa-spin"></i></td></tr>';
    ...
    try {
        ...
        // 🔥 جلب طلاب هذا المعلم فقط من السحابة 🔥
                ...
                <tr>
                    <td colspan="8" style="text-align: center; padding: 40px;">
                        <div class="empty-state">
                            ...
                            <h3>لا يوجد طلاب</h3>
                            <p>لم يقم المعلم (أ. ${teacherName}) بإضافة أي طلاب حتى الآن.</p>
                        </div>
                    </td>
                </tr>
            `;
        ...
        tableBody.innerHTML = '<tr><td colspan="8" class="text-danger text-center">حدث خطأ أثناء جلب الطلاب من قاعدة البيانات</td></tr>';
    }
}

// رسم الجدول
function renderStudentsTable(studentsArray, teacherName) {
    const tableBody = document.getElementById('studentsTableBody');
    if (!tableBody) return;
    ...
    tableBody.innerHTML = studentsArray.map(student => {
        ...
        return `
            <tr data-student-id="${student.id}" data-grade="${student.grade || ''}" data-subject="${student.subject || ''}" data-progress="${progress}">
                <td>
...
function filterStudents() {
    ...
    const teacherName = document.getElementById('teacherFilter').options[0].text.replace('أ. ', '');
    renderStudentsTable(filtered, teacherName);
}
...
function toggleSelectAll(forceState = null) {
    ...
    document.querySelectorAll('.student-checkbox').forEach(checkbox => {
        ...
        if (isChecked) { selectedStudents.add(studentId); if(row) row.classList.add('selected'); } 
        else { selectedStudents.delete(studentId); if(row) row.classList.remove('selected'); }
    });
    updateSelectedCount();
}
...
function toggleStudentSelection(studentId) {
    ...
    if (checkbox && checkbox.checked) { selectedStudents.add(studentId); if(row) row.classList.add('selected'); } 
    else { selectedStudents.delete(studentId); if(row) row.classList.remove('selected'); }
    ...
    if(selectAllCheckbox) selectAllCheckbox.checked = (allVisible > 0 && allVisible === allChecked);
}

function selectAllStudents() { toggleSelectAll(true); }
...
function updateSelectedCount() {
    ...
    if (generateBtn) generateBtn.innerHTML = count > 0 ? `<i class="fas fa-print"></i> توليد تقرير (${count} طالب)` : `<i class="fas fa-print"></i> توليد تقرير للمحددين`;
}
...
function generateStudentReport(studentId) {
    selectedStudents.clear(); selectedStudents.add(studentId); currentReportStudentIds = [studentId];
    ...
    updateSelectedCount(); showReportOptions();
}

function showReportOptions() {
    const optionsSection = document.getElementById('reportOptionsSection');
    if(optionsSection) { optionsSection.style.display = 'block'; optionsSection.scrollIntoView({ behavior: 'smooth' }); }
}
...
// 🌟 الدالة التي تستدعي محرك التقارير السحابي
window.initiateReport = window.memberGenerateReport = async function() {
    ...
    try {
        // استدعاء دوال التوليد من ملف reports.js
        ...
        else previewArea.innerHTML = `<div class="alert alert-warning">ملف reports.js غير متصل. يرجى مراجعة الخطوة الثانية.</div>`;
...
function saveReportLog(type, count) {
    ...
    committeeReports.push({ id: Date.now(), committeeId: currentUser.id, reportType: type, studentCount: count, createdAt: new Date().toISOString() });
...
function loadGeneratedReports() {
    ...
    const currentUser = getCurrentUser();
    const committeeReports = JSON.parse(localStorage.getItem('committeeReports') || '[]');
    const userReports = committeeReports.filter(cr => cr.committeeId === currentUser.id).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 10);
    
    if (userReports.length === 0) {
        reportsList.innerHTML = `<div class="empty-state" style="text-align:center; padding:20px; color:#777;"><div style="font-size:2rem; margin-bottom:10px;">📋</div><p>لا توجد تقارير منشأة مسبقاً</p></div>`;
        return;
    }
    
    const typeNames = { 'attendance':'الغياب', 'achievement':'الإنجاز', 'assignments':'الواجبات', 'iep':'الخطط الفردية', 'diagnostic':'الاختبار التشخيصي', 'schedule':'الجدول الدراسي', 'balance': 'رصيد الحصص', 'studentData': 'بيانات الطلاب' };
