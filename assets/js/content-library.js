    ...
    if (multiTypes.includes(type)) {
        ...
        if (type === 'mcq' || type === 'mcq-media') {
            ...
            if (type === 'mcq-media') {
                ...
                html += `<div class="form-group mb-3 p-2 bg-light border rounded"><label class="q-label"><i class="fas fa-paperclip"></i> مرفق (صورة)</label><input type="file" class="form-control-file q-attachment"><input type="hidden" class="q-existing-attachment" value="${existingFile}">${existingFile ? `<div class="attachment-preview mt-2"><img src="${existingFile}" style="max-height:80px; border:1px solid #ddd;"></div>` : ''}</div>`;
            }
            ...
            html += `<div class="form-group mt-2"><label class="q-label">الإجابة النموذجية (اختياري)</label><textarea class="form-control q-model-answer" rows="2">${data?.modelAnswer || ''}</textarea></div>`;
        }
        area.innerHTML = html;
    }
}
...
function addParagraphInput(qUniqueId, type, itemData = null) {
    ...
    div.style.cssText = "position:relative; background:#fff; border:1px solid #eee; padding:15px; border-radius:8px; margin-bottom:15px;";
...
function initDragHighlighter(qUniqueId, pIdx, savedGaps = null) {
    ...
    area.innerHTML = `<div style="margin-bottom:10px;"><p id="sel-text-${qUniqueId}-${pIdx}" style="font-size:1.3rem; letter-spacing:1px; line-height:1.8;">${text}</p></div><button type="button" class="btn btn-warning btn-sm" onclick="markGap('${qUniqueId}', '${pIdx}')"><i class="fas fa-highlighter"></i> تحويل المحدد لفراغ</button><button type="button" class="btn btn-secondary btn-sm" onclick="resetGap('${qUniqueId}', '${pIdx}')">إعادة تعيين</button><div id="gap-prev-${qUniqueId}-${pIdx}" class="gap-preview mt-2"></div>`;
    if(savedGaps) {
