...
// ==========================================
// 1. الاختبارات التشخيصية (عرض + ربط فقط)
// ==========================================
function loadTests() {
...
function showExportModal() {
    ...
    const categories = [
        { key: 'tests', label: 'الاختبارات التشخيصية' },
        { key: 'lessons', label: 'الدروس التفاعلية' },
        { key: 'objectives', label: 'الأهداف قصيرة المدى' },
        { key: 'homeworks', label: 'الواجبات' }
    ];
    ...
    categories.forEach(cat => {
        ...
        if (items.length > 0) {
            let html = `
                <div class="export-section">
                    <div class="export-section-header">
                        <span>${cat.label}</span>
                        <div>
                            <input type="checkbox" onchange="toggleCategorySelect(this, '${cat.key}')" id="cat_all_${cat.key}">
                            <label for="cat_all_${cat.key}" style="font-size:0.8rem; cursor:pointer;">الكل</label>
                        </div>
                    </div>
            ...
            items.forEach(item => {
                const title = item.title || item.shortTermGoal;
                html += `
                    <div class="export-item">
                        <input type="checkbox" class="export-check-item ${cat.key}-item" value="${item.id}" data-category="${cat.key}">
                        <label>${title}</label>
                    </div>
                `;
            });
    ...
    if(container.innerHTML === '') {
        container.innerHTML = '<p class="text-center text-muted">المكتبة فارغة، لا يوجد محتوى للتصدير.</p>';
    }
...
function toggleCategorySelect(checkbox, category) {
    document.querySelectorAll(`.${category}-item`).forEach(cb => cb.checked = checkbox.checked);
}

function toggleGlobalSelect(checkbox) {
    document.querySelectorAll('.export-check-item').forEach(cb => cb.checked = checkbox.checked);
    document.querySelectorAll('[id^="cat_all_"]').forEach(cb => cb.checked = checkbox.checked);
}
...
function executeExport() {
    const exportData = {
        ...
        lessons: [],
        objectives: [],
        assignments: [], // homeworks stored as assignments
        version: "1.0",
    ...
    document.querySelectorAll('.tests-item:checked').forEach(cb => {
        const item = tests.find(t => t.id == cb.value);
        if(item) {
            const clean = JSON.parse(JSON.stringify(item));
            if(clean.questions) clean.questions.forEach(q => delete q.linkedGoalId); // تنظيف
            exportData.tests.push(clean);
        }
    });

    // جمع الدروس
    const lessons = JSON.parse(localStorage.getItem('lessons') || '[]');
    document.querySelectorAll('.lessons-item:checked').forEach(cb => {
        const item = lessons.find(l => l.id == cb.value);
        if(item) {
            const clean = JSON.parse(JSON.stringify(item));
            delete clean.linkedInstructionalGoal; // تنظيف
            exportData.lessons.push(clean);
        }
    });

    // جمع الأهداف
    const objectives = JSON.parse(localStorage.getItem('objectives') || '[]');
    document.querySelectorAll('.objectives-item:checked').forEach(cb => {
        const item = objectives.find(o => o.id == cb.value);
        if(item) exportData.objectives.push(item);
    });

    // جمع الواجبات
    const homeworks = JSON.parse(localStorage.getItem('assignments') || '[]');
    document.querySelectorAll('.homeworks-item:checked').forEach(cb => {
        const item = homeworks.find(h => h.id == cb.value);
        if(item) {
            const clean = JSON.parse(JSON.stringify(item));
            delete clean.linkedInstructionalGoal; // تنظيف
            exportData.assignments.push(clean);
        }
    });
    ...
    if(totalCount === 0) {
        alert('لم تقم بتحديد أي عنصر للتصدير.');
        return;
    }
    ...
    a.href = url;
    a.download = `ContentLibrary_Export_${new Date().toISOString().slice(0,10)}.json`;
...
function importContent(input) {
    ...
    reader.onload = function(e) {
        try {
            ...
            // دالة مساعدة لإضافة عنصر
            const addItem = (key, item) => {
                const arr = JSON.parse(localStorage.getItem(key) || '[]');
                // تنظيف وتجهيز
                item.id = Date.now() + Math.floor(Math.random() * 1000); // ID جديد
                item.teacherId = teacherId;
                if(item.questions) item.questions.forEach(q => delete q.linkedGoalId);
                delete item.linkedInstructionalGoal;
            ...
            // 1. فحص هل الملف مجمع (Unified)
            if (data.version && (data.tests || data.lessons || data.objectives || data.assignments)) {
                if(data.tests) data.tests.forEach(i => addItem('tests', i));
                if(data.lessons) data.lessons.forEach(i => addItem('lessons', i));
                if(data.objectives) data.objectives.forEach(i => addItem('objectives', i));
                if(data.assignments) data.assignments.forEach(i => addItem('assignments', i)); // الواجبات
            } 
            // 2. فحص هل الملف مفرد (Legacy/Single Export)
            else if (data.contentType) {
                 if (data.contentType === 'test') addItem('tests', data);
                 else if (data.contentType === 'lesson') addItem('lessons', data);
                 else if (data.contentType === 'homework') addItem('assignments', data);
                 else if (data.contentType === 'objective') addItem('objectives', data);
            }
            // 3. محاولة تخمين النوع للملفات القديمة جداً
            else {
                 const type = guessContentType(data);
                 if(type === 'objective') addItem('objectives', data);
                 else if(type === 'lesson') addItem('lessons', data);
                 else if(type === 'test') addItem('tests', data);
                 else if(type === 'homework') addItem('assignments', data);
                 else { alert('تعذر التعرف على نوع الملف.'); return; }
            }

            alert(`تم استيراد ${count} عنصر بنجاح!`);
            loadContentLibrary(); // تحديث الواجهة
...
function guessContentType(data) {
    ...
    if (data.questions && data.description !== undefined && !data.intro) return 'test';
    if (data.questions && !data.intro) return 'homework';
    return null;
}

// ==========================================
// دوال الربط (كما هي في الرد السابق)
// ==========================================
...
// ==========================================
// دوال المودالات (Add/Edit/Delete) - الأساسية
// ==========================================
// دوال الاختبارات
