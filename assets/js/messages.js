...
// الوصف: نسخة الكمبيوتر الأصلية (لم يتم المساس بها) + تعديلات الجوال معزولة تماماً في الأسفل
// ============================================
...
function injectChatStyles() {
    ...
    style.innerHTML = `
        /* =========================================
           💻 تنسيقات الكمبيوتر (تم استعادتها من نسختك الأصلية بالضبط)
        ========================================= */
        ...
        /* حاويات أضفتها لتسهيل تعديل الجوال، لكنها في الكمبيوتر تعمل كـ display: contents لكي لا تؤثر على تصميمك الأساسي */
        .input-tools-wrapper { display: contents; }
        .input-main-wrapper { display: contents; }
        ...
        .btn-tool { width: 45px; height: 45px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.2rem; cursor: pointer; transition: 0.2s; border: none; color: white; box-shadow: 0 2px 5px rgba(0,0,0,0.2); flex-shrink: 0; }
        ...
        /* إخفاء زر قائمة الجوال من شاشة الكمبيوتر */
        .mobile-only-btn { display: none; }


        /* =========================================
           📱 تخصيص الجوال حصراً (أقل من 768px)
           هنا فقط يتم تفعيل التعديلات المعزولة تماماً
        ========================================= */
        @media (max-width: 768px) { 
            .mobile-only-btn { display: inline-block; background: transparent; border: none; font-size: 1.2rem; color: #333; margin: 0 10px; cursor: pointer; }
            
            /* تمديد المحادثة أسفل الشاشة */
            .chat-container { height: 100vh !important; border-radius: 0; border: none; margin: 0; }
            .messages-container { height: calc(100vh - 120px) !important; margin-bottom: -30px !important; }
            
            /* القائمة الجانبية في الجوال */
            .chat-sidebar { position: fixed; right: -100%; top: 0; height: 100%; width: 280px; z-index: 9999; transition: right 0.3s ease; box-shadow: -4px 0 15px rgba(0,0,0,0.1); }
            .chat-sidebar.show-contacts { right: 0; }

            /* الرأس (إخفاء PDF وتعديل زر الحذف) */
            .chat-header { height: 60px; padding: 5px 10px; }
            .chat-header .avatar { width: 35px; height: 35px; font-size: 0.9rem; }
            .header-info { min-width: 0; }
            #chatHeaderName { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 140px; }
            
            /* إخفاء زر الـ PDF في الجوال فقط بناءً على طلبك */
            ...
            /* ترتيب منطقة الكتابة (مربعات موسطة للمرفقات) */
            .chat-input-area { flex-direction: column; align-items: stretch; padding: 10px; gap: 8px; background: #f0f2f5; }
            .input-main-wrapper { display: flex; align-items: center; gap: 8px; width: 100%; order: 1; }
            .input-tools-wrapper { display: flex; justify-content: center; gap: 15px; width: 100%; order: 2; padding-top: 5px; }
            
            .chat-input { margin: 0; padding: 10px 15px; border-radius: 20px; }
            .btn-send-pill { padding: 8px 15px; }
            
            /* تحويل دوائر المرفقات إلى مربعات */
        ...
        <div class="chat-container">
            <div class="chat-sidebar" id="chatSidebar">
                <div class="chat-list-header" style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <button class="mobile-only-btn" onclick="document.getElementById('chatSidebar').classList.remove('show-contacts')">
            ...
            <div class="chat-main">
                <div class="chat-header" id="chatHeader" style="display:none;">
                    <div class="header-info">
                        <button class="mobile-only-btn" onclick="document.getElementById('chatSidebar').classList.add('show-contacts')">
                ...
                <div class="messages-area" id="chatMessagesArea">
                    <div class="empty-chat">
                        ...
                        <button class="mobile-only-btn" style="margin-top:20px; border-radius:25px; padding:10px 20px; font-weight:bold; background:var(--primary-color, #007bff); color:white;" onclick="document.getElementById('chatSidebar').classList.add('show-contacts')">
                            👥 إظهار قائمة الطلاب
                        </button>
                    </div>
                </div>
...
function confirmDeleteAction() {
    ...
    document.getElementById('chatMessagesArea').innerHTML = `<div class="empty-chat"><i class="far fa-comments fa-4x mb-4" style="color:#cbd5e1;"></i><p style="font-size:1.1rem;">اختر طالباً للبدء بالمراسلة</p><button class="mobile-only-btn" style="margin-top:20px; border-radius:25px; padding:10px 20px; font-weight:bold; background:var(--primary-color, #007bff); color:white;" onclick="document.getElementById('chatSidebar').classList.add('show-contacts')">👥 إظهار قائمة الطلاب</button></div>`;
}
