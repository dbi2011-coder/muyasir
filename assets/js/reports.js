...
// الوصف: نظام التقارير الشامل (تم تعديل CSS لضمان ظهور التذييل عند الطباعة)
// ============================================
...
(function injectPrintStyles() {
    ...
    style.innerHTML = `
        ...
        .custom-footer {
            ...
            width: 100%;
            text-align: center;
            font-size: 12pt; /* تكبير الخط قليلاً */
            font-weight: bold;
            color: #000 !important;
        ...
        @media print {
            @page {
                size: A4;
                margin: 10mm; 
                margin-bottom: 30mm; /* 🔥 حجز مساحة كبيرة في الأسفل للتذييل */
            }
            ...
            #reportPreviewArea {
                ...
                position: relative !important; /* تغيير من absolute لضمان تدفق المحتوى */
                width: 100% !important;
            ...
            /* 🔥 تنسيق التذييل عند الطباعة - حل مشكلة الاختفاء */
            .custom-footer {
                position: fixed !important; /* تثبيت في أسفل كل صفحة */
                bottom: 0 !important;
                left: 0 !important;
                right: 0 !important;
                width: 100% !important;
                ...
                padding-bottom: 15px; /* مسافة أمان من حافة الورقة */
                page-break-after: always; /* ضمان عدم تداخل الصفحات */
            }
            ...
            /* إصلاح الألوان والرسوم البيانية */
...
window.initiateReport = function() {
    ...
    try {
        ...
        else previewArea.innerHTML = `<div class="alert alert-warning text-center no-print">عفواً، هذا التقرير قيد التطوير.</div>`;
    } catch (error) {
        console.error("Report Error:", error);
        previewArea.innerHTML = `<div class="alert alert-danger text-center">خطأ: ${error.message}</div>`;
    }
};
...
function generateAttendanceReport(studentIds, container) {
    ...
    html += `</tbody></table>
            ...
            <div class="mt-4 text-left no-print"><button onclick="window.print()" class="btn btn-primary">طباعة التقرير 🖨️</button></div>
        </div>`;
...
function generateIEPReport(studentIds, container) {
    ...
    studentIds.forEach((id, idx) => {
        ...
        html += `
        <div class="student-iep-page" style="page-break-after:always; position:relative; min-height:95vh;">
    ...
    html += `<div class="mt-4 text-left no-print" style="padding:20px;"><button onclick="window.print()" class="btn btn-primary">طباعة التقارير 🖨️</button></div></div>`;
    container.innerHTML = html;
}
...
function generateDiagnosticReport(studentIds, container) {
    ...
    studentIds.forEach((id, idx) => {
        ...
        html += `
        <div class="student-diagnostic-page" style="page-break-after:always; position:relative; min-height:95vh;">
