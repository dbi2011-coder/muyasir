...
// الوصف: عرض الخطة للطالب معتمدة كلياً على السحابة
// ============================================
...
async function loadStudentIEP() {
    ...
    iepContainer.innerHTML = '<div class="text-center p-5"><h3><i class="fas fa-spinner fa-spin"></i> جاري جلب الخطة من السحابة...</h3></div>';

    try {
        const [testsRes, allTestsRes, objsRes, schedRes, lessonsRes] = await Promise.all([
            window.supabase.from('student_tests').select('*').eq('studentId', currentStudentId).eq('type', 'diagnostic').eq('status', 'completed'),
        ...
        const teacherSchedule = schedRes.data || [];
        const studentLessons = lessonsRes.data || [];

        const completedDiagnostic = studentTests.sort((a, b) => new Date(b.assignedDate) - new Date(a.assignedDate))[0];
        ...
        <div class="iep-word-model-content" style="background:#fff; padding:20px; border:1px solid #ccc; font-family:'Tajawal', sans-serif; box-shadow: 0 4px 6px rgba(0,0,0,0.1); border-radius:8px;">
            ...
            <div class="print-footer-container">
                <p class="print-footer-text">
                    تم طباعة الخطة التربوية الفردية من منصة ميسر التعلم لمعلم صعوبات التعلم
                </p>
            </div>
        </div>
        ...
        console.error("Error loading IEP:", e);
        iepContainer.innerHTML = '<div class="alert alert-danger">حدث خطأ أثناء الاتصال بالخادم. يرجى المحاولة لاحقاً.</div>';
    }
}
