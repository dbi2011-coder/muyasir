...
// الوصف: شات الطالب (محدث بالأيقونات الملونة والتسجيل)
// ============================================
...
// نفس ستايل المعلم (مع الأزرار الملونة)
function injectChatStyles() {
    ...
    style.innerHTML = `
        ...
        .chat-input { flex: 1; padding: 12px 15px; border: 2px solid #e2e8f0; border-radius: 25px; outline: none; font-size: 1rem; background: #f8fafc; }
        ...
        .btn-emoji { background: #fff9c4; color: #f57f17; }
        .btn-attach { background: #e0f2f1; color: #00897b; }
        .btn-cam { background: #e3f2fd; color: #1565c0; }
        .btn-mic { background: #ffebee; color: #c62828; }
        ...
        <div class="chat-container">
            <div class="chat-main">
                ...
                <div class="chat-input-area">
                    ...
                    <button class="btn-tool" onclick="cancelEdit()" id="cancelEditBtn" style="display:none; color:#dc3545;" title="إلغاء"><i class="fas fa-times"></i></button>
...
function loadChatWithTeacher() {
    ...
    myMsgs.forEach(msg => {
        ...
        const html = `<div class="msg-bubble ${bubbleClass}">
                        ${menuHtml}
...
function startRecording() {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        alert('المتصفح لا يدعم التسجيل'); return;
    }
...
function cancelRecording() {
    if (mediaRecorder && mediaRecorder.state === 'recording') {
        mediaRecorder.onstop = null; // إلغاء الإرسال
        mediaRecorder.stop();
        clearInterval(recordingInterval);
        document.getElementById('recordingArea').style.display = 'none';
    }
