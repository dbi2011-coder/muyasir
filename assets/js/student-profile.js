...
// الوصف: ملف الطالب للمعلم + تصحيح المعلم للرسم الكتابي وغيره
// ============================================
...
function buildTeacherReviewItem(q, index, studentAnsObj) {
    ...
    if (q.type === 'handwriting') { // 🔥 مراجعة الرسم الكتابي (النسخ) وتصحيح المعلم
        ...
        (q.paragraphs || []).forEach((p, pIdx) => {
            ...
            if(studentImg) {
                ...
                <div class="correction-toolbar mb-3" style="display:flex; justify-content:center; gap:10px; flex-wrap:wrap; background:#f8f9fa; padding:10px; border-radius:8px;">
                    <button type="button" class="btn btn-sm btn-outline-danger" id="tc-pen-${q.id}-${pIdx}" onclick="setCorrectionMode('${q.id}', '${pIdx}', 'pen')"><i class="fas fa-pen"></i> قلم أحمر</button>
    ...
    try {
        ...
        if(questions && questions.length > 0) {
            questions.forEach(q => {
                ...
                // جمع التصحيح للرسم الكتابي إذا وجد
...
// ... بقية دوال student-profile.js القياسية (مثل الإرجاع، الجداول، إلخ) تبقى كما هي من الكود السابق
...
// ... (دوال الإداريات والتقدم كما هي) ...

// ============================================
// الدوال الباقية الخاصة بالتبويبات الأخرى (التشخيص والتقدم) مقتطعة للاختصار ولكن يجب أن تضعها كما كانت في ملفك الأساسي.
// ============================================

// [ضع هنا بقية دوال loadDiagnosticTab, loadIEPTab, loadLessonsTab ... إلخ المذكورة في ردودي السابقة]
