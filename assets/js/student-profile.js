...
function loadIEPTab() {
    ...
    if (originalTest && originalTest.questions) { originalTest.questions.forEach(q => { const ans = completedDiagnostic.answers ? completedDiagnostic.answers.find(a => a.questionId == q.id) : null; const score = ans ? (ans.score || 0) : 0; if (q.linkedGoalId) { const obj = allObjectives.find(o => o.id == q.linkedGoalId); if (obj) { if (score >= (q.passingScore || 1)) { if (!strengthHTML.includes(obj.shortTermGoal)) strengthHTML += `<li>${obj.shortTermGoal}</li>`; } else { if (!needsObjects.find(o => o.id == obj.id)) { needsObjects.push(obj); needsHTML += `<li>${obj.shortTermGoal}</li>`; } } } } }); }
    if(!strengthHTML) strengthHTML = '<li>لا توجد</li>'; if(!needsHTML) needsHTML = '<li>لا توجد</li>';
    ...
    needsObjects.forEach(obj => { objectivesRows += `<tr style="background-color:#dbeeff !important;"><td class="text-center" style="font-weight:bold; color:#0056b3;">${stgCounter++}</td><td colspan="2" style="font-weight:bold; color:#0056b3;">الهدف: ${obj.shortTermGoal}</td></tr>`; if (obj.instructionalGoals) obj.instructionalGoals.forEach(iGoal => { const compDate = completedLessonsMap[iGoal], accelDate = acceleratedLessonsMap[iGoal]; let dateDisplay = '', rowStyle = ''; if (accelDate) { dateDisplay = `<span style="font-weight:bold; color:#856404;">⚡ ${new Date(accelDate).toLocaleDateString('ar-SA')} (تفوق)</span>`; rowStyle = 'background-color:#fff3cd !important;'; } else if (compDate) { dateDisplay = `<span class="text-success font-weight-bold">✔ ${new Date(compDate).toLocaleDateString('ar-SA')}</span>`; } else { dateDisplay = `<span style="color:#ccc;">--/--/----</span>`; } objectivesRows += `<tr style="${rowStyle}"><td class="text-center">-</td><td>${iGoal}</td><td>${dateDisplay}</td></tr>`; }); });
    const teacherSchedule = JSON.parse(localStorage.getItem('teacherSchedule') || '[]'); const dayKeys = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday'];
    let scheduleCells = dayKeys.map(dk => { const session = teacherSchedule.find(s => s.day === dk && (s.studentId == currentStudentId || (s.students && s.students.includes(currentStudentId)))); return `<td style="height:50px;">${session ? 'حصة ' + (session.period||1) : ''}</td>`; }).join('');
    iepContainer.innerHTML = `<div class="iep-printable" style="background:#fff; padding:20px; border:1px solid #ccc;"><h3 style="text-align:center;">الخطة التربوية</h3><table class="table table-bordered mb-4"><tr><td style="background:#f5f5f5;">الطالب:</td><td>${currentStudent.name}</td></tr></table><h5>جدول الحصص:</h5><table class="table table-bordered text-center mb-4"><thead><tr style="background:#f5f5f5;"><th>الأحد</th><th>الاثنين</th><th>الثلاثاء</th><th>الأربعاء</th><th>الخميس</th></tr></thead><tbody><tr>${scheduleCells}</tr></tbody></table><div style="display:flex; gap:20px;"><div style="flex:1;"><h6 style="background:#28a745; color:white; padding:5px;">نقاط القوة</h6><ul>${strengthHTML}</ul></div><div style="flex:1;"><h6 style="background:#dc3545; color:white; padding:5px;">نقاط الاحتياج</h6><ul>${needsHTML}</ul></div></div><h5 class="mt-4">الأهداف:</h5><table class="table table-bordered"><thead><tr><th>#</th><th>الهدف</th><th>التحقق</th></tr></thead><tbody>${objectivesRows}</tbody></table></div>`;
...
function loadLessonsTab() {
    ...
    container.innerHTML = myList.map((l,i) => {
        ...
        return `<div class="content-card"><h4>${i+1}. ${l.title}</h4><div>${l.status}</div><div>${controls}</div></div>`;
    }).join('');
}
// الدوال المساعدة (Modal, Lessons, etc.)
function openReviewModal(id) { document.getElementById('reviewAssignmentId').value = id; document.getElementById('reviewTestModal').classList.add('show'); } 
function saveTestReview() { /* ... */ alert('تم الحفظ'); closeModal('reviewTestModal'); loadDiagnosticTab(); }
