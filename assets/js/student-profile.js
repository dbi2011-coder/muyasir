...
// الوصف: إدارة ملف الطالب (الترتيب الرقمي + التحكم الكامل + إصلاح التواريخ)
// ============================================
        ...
        if(assignedTest.status === 'completed') {
            ...
            actionContent = `
                <div style="margin-top:15px; padding:15px; background:#f0fff4; border:1px solid #c3e6cb; border-radius:5px;">
                    <strong>الدرجة: ${assignedTest.score || 0}%</strong>
...
// ---------------------------------------------------------
// 3. قسم الدروس (ترتيب رقمي + تحكم كامل)
// ---------------------------------------------------------
function loadLessonsTab() {
    ...
    // الترتيب: حسب orderIndex (الرقم اليدوي) إن وجد، وإلا حسب التاريخ
    myList.sort((a, b) => {
...
function updateLessonOrder(lessonId, newOrder) {
    ...
    if (idx !== -1) {
        ...
        loadLessonsTab(); // إعادة التحميل لتطبيق الترتيب الجديد فوراً
    }
}

// ---------------------------------------------------------
// دوال التحكم بالدروس (مشاهدة، تصفير، قفل)
// ---------------------------------------------------------
...
function resetLesson(id) {
    if(!confirm('هل أنت متأكد من إعادة فتح الدرس؟ سيتم حذف إجابات الطالب وتاريخ الإنجاز.')) return;
    const studentLessons = JSON.parse(localStorage.getItem('studentLessons') || '[]');
