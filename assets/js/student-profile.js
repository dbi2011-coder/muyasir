...
function loadProgressTab() {
    ...
    if (myList.length > 0) {
        // ترتيب الدروس حسب تاريخ التعيين للحصول على أقدم تاريخ
        ...
        if (l.historyLog && l.historyLog.length > 0) {
            l.historyLog.forEach(log => {
                rawLogs.push({
                    ...
                    dateStr: new Date(log.date).toDateString(), // للمقارنة (بدون وقت)
                    type: 'lesson',
                    status: log.status, // started, extension, completed, absence, accelerated
                    title: l.title,
                    lessonId: l.id,
                    cachedType: log.cachedSessionType || null // ✅ هنا نتحقق من الأرشفة (Snapshotted Type)
                });
            });
        }
    });
    ...
    // 3. المعالجة الزمنية (يوم بيوم) من البداية وحتى اليوم
    let finalTimeline = [];
    let balance = 0; // ⚖️ الرصيد التراكمي (0 = متوازن)
    const today = new Date();
    const dayMap = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'];

    // حلقة التكرار الرئيسية (Main Loop)
    for (let d = new Date(planStartDate); d <= today; d.setDate(d.getDate() + 1)) {
        const currentDateStr = d.toDateString();
        const currentISODate = d.toISOString();
        ...
        // 🧹 تنظيف العرض (Filtering Logic) 🧹
        // إذا تحقق الدرس في نفس اليوم، نحذف "started"
        ...
        if (completedIdsToday.length > 0) {
            daysLogs = daysLogs.filter(l => {
                // إذا كان "بدأ" لنفس الدرس المكتمل اليوم، احذفه
                if (l.status === 'started' && completedIdsToday.includes(l.lessonId)) return false;
                return true;
            });
        }

        // إذا لم يكن هناك أي سجلات في يوم مجدول -> تسجيل غياب تلقائي
        ...
        // 🧮 معالجة كل سجل في هذا اليوم وحساب الرصيد 🧮
        daysLogs.forEach(log => {
            let displayStatus = '';
            let displayType = ''; // أساسية، تعويضية، إضافية
            let rowClass = '';
            let studentState = ''; // حاضر، غائب، ...
            ...
            // --- حالة 3: حضور درس (started, extension, completed) ---
            } else {
                studentState = 'حاضر';
                
                // تحديد مسمى حالة الدرس
                ...
                // 🔥 تحديد نوع الحصة (أساسية / تعويضية / إضافية)
                // الأولوية 1: هل النوع مخزن سابقاً (Snapshot)؟
                if (log.cachedType) {
                    if (log.cachedType === 'basic') displayType = 'أساسية';
                    else if (log.cachedType === 'compensation') { displayType = '<span class="text-primary font-weight-bold">تعويضية</span>'; balance++; } // سداد دين
                    else if (log.cachedType === 'additional') { displayType = 'إضافية'; balance++; } // زيادة رصيد
                
                // الأولوية 2: الحساب المباشر (إذا لم يكن هناك أرشفة)
                } else {
                    if (isScheduledDay) {
                        displayType = 'أساسية';
                        // في الحصة الأساسية لا نغير الرصيد (0)
                    } else {
                        // يوم غير مجدول
                        if (balance < 0) {
            ...
            // إضافة السجل للقائمة النهائية للعرض
            finalTimeline.push({
                title: log.title,
                ...
                rawDate: d, // للترتيب
                balanceSnapshot: balance, // لعرض الدين بجانب الحالة
    ...
    // 4. بناء الجدول (Rendering)
    // الترتيب: الأحدث في الأسفل (تراكمي) أو في الأعلى حسب رغبتك (هنا سأجعله تراكمياً زمنياً)
