...
// الوصف: ملف الطالب للمعلم + تصحيح المعلم للرسم الكتابي (سحابي 100% ومستقر)
// ============================================
...
// =========================================================
// 🔥 1. دوال النوافذ المنبثقة (Global UI) 🔥
// =========================================================
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!currentStudentId) { 
        window.showError('لم يتم تحديد طالب'); 
...
window.loadStudentData = async function() {
    try {
        ...
        await window.calculateAndSetStudentProgress();
        window.switchSection('diagnostic');
    } catch(e) {
        console.error(e);
        window.showError('حدث خطأ في جلب بيانات الطالب');
...
window.switchSection = function(sectionId) {
    ...
    if (sectionId === 'diagnostic') window.loadDiagnosticTab();
    if (sectionId === 'iep') window.loadIEPTab();
    if (sectionId === 'lessons') window.loadLessonsTab();
    if (sectionId === 'assignments') window.loadAssignmentsTab();
    if (sectionId === 'progress') window.loadProgressTab();
};

window.calculateAndSetStudentProgress = async function() {
    try {
...
// =========================================================
// 🔥 4. التبويب: الاختبار التشخيصي 🔥
// =========================================================
window.loadDiagnosticTab = async function() {
    try {
            ...
            if(assignedTest.status === 'completed') {
                ...
                actionContent = `<div style="margin-top:15px; padding:15px; background:#f0fff4; border:1px solid #c3e6cb; border-radius:8px;">
                    ...
                    <div style="margin-top:15px; display:flex; gap:10px;">
                        <button class="btn btn-warning" onclick="window.openReviewModal(${assignedTest.id}, 'test')">🔍 مراجعة وتصحيح</button>
                        <button class="btn btn-primary" onclick="window.autoGenerateLessons()">⚡ توليد الخطة والدروس</button>
                    </div>
                </div>`;
                ...
                <div class="card" style="border:1px solid #eee; box-shadow:0 4px 10px rgba(0,0,0,0.05); padding:20px;">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        ...
                        <div style="display:flex; gap:5px; align-items:center;">
                            ${statusBadge}
                            <button class="btn btn-sm btn-outline-danger" onclick="window.deleteAssignedTest(${assignedTest.id})" title="حذف الاختبار"><i class="fas fa-trash"></i></button>
                        </div>
                    </div>
...
window.showAssignTestModal = async function() { 
...
window.assignTest = async function() { 
    ...
    try {
        ...
        await window.supabase.from('student_tests').insert([{ id: Date.now(), studentId: currentStudentId, testId: testId, type: 'diagnostic', status: 'pending', assignedDate: new Date().toISOString() }]);
        window.closeModal('assignTestModal'); 
        window.loadDiagnosticTab(); 
        window.showSuccess('تم تعيين الاختبار بنجاح.');
    } catch(e) { console.error(e); alert('حدث خطأ'); }
};

window.deleteAssignedTest = async function(id) { 
    window.showConfirmModal('هل أنت متأكد من حذف هذا الاختبار المعين؟', async function() {
        await window.supabase.from('student_tests').delete().eq('id', id);
        window.loadDiagnosticTab(); 
        window.showSuccess('تم الحذف بنجاح.'); 
    });
};

// =========================================================
// 🔥 5. التبويب: الخطة التربوية (IEP) 🔥
// =========================================================
window.loadIEPTab = async function() {
    ...
    try {
        ...
        const { data: allObjectives } = await window.supabase.from('objectives').select('*').eq('teacherId', window.getCurrentUser().id);
...
// =========================================================
// 🔥 6. التبويب: إدارة الدروس 🔥
// =========================================================
window.regenerateLessons = function() { window.autoGenerateLessons(); };

window.loadLessonsTab = async function() {
    try {
        ...
        if (!myLessons || myLessons.length === 0) { container.innerHTML = `<div class="empty-state"><h3>لا توجد دروس</h3><button class="btn btn-primary" onclick="window.autoGenerateLessons()">⚡ توليد تلقائي</button></div>`; return; }
        ...
        container.innerHTML = myLessons.map((l, index) => {
            ...
            if (l.status === 'completed') { statusBadge = l.passedByAlternative ? '<span class="badge badge-info">✅ مجتاز ببديل</span>' : '<span class="badge badge-success">✅ مكتمل</span>'; cardStyle = 'border-right: 5px solid #28a745;'; } 
            ...
            if (l.status === 'completed' || l.status === 'accelerated') {
                controls = `<button class="btn btn-outline-success btn-sm w-100 mb-2" onclick="window.openReviewModal(${l.id}, 'lesson')">🔍 مراجعة الإجابات</button><button class="btn btn-warning btn-sm" onclick="window.resetLesson(${l.id})">🔄 إعادة فتح</button>`;
            } else if (l.status === 'pending_review') {
                controls = `<button class="btn btn-warning btn-sm w-100 mb-2" style="color:#000; font-weight:bold;" onclick="window.openReviewModal(${l.id}, 'lesson')">📝 تصحيح ورصد الدرجة</button><button class="btn btn-danger btn-sm" onclick="window.resetLesson(${l.id})">إعادة فتح</button>`;
            } else {
                controls = `<button class="btn btn-info btn-sm" style="background:#ffc107; border:none; color:#000;" onclick="window.accelerateLesson(${l.id})">⚡ تسريع (تفوق)</button>`;
            }

            const isFirst = index === 0; const isLast = index === myLessons.length - 1;
            let orderBtns = '';
            if (!isFirst) orderBtns += `<button class="btn-order" style="width:auto; height:auto; padding:2px 8px; border-radius:4px; margin-left:5px;" onclick="window.moveLesson(${l.id}, 'up')">تقديم</button>`;
            if (!isLast) orderBtns += `<button class="btn-order" style="width:auto; height:auto; padding:2px 8px; border-radius:4px;" onclick="window.moveLesson(${l.id}, 'down')">تأخير</button>`;

            return `<div class="content-card" style="${cardStyle} position:relative;"><div style="position:absolute; top:50px; left:10px; display:flex; z-index:5;">${orderBtns}</div><div style="display:flex; justify-content:space-between;"><div style="margin-right:20px;"><h4 style="margin:0;">${index+1}. ${l.title}</h4><small class="text-muted">${l.objective}</small></div><div>${statusBadge}</div></div><div style="margin-top:10px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap;"><div class="lesson-actions" style="width:100%; display:flex; gap:5px; margin-top:25px;">${controls}<button class="btn btn-danger btn-sm" onclick="window.deleteLesson(${l.id})">حذف</button></div></div></div>`;
        }).join('');
    } catch(e) { console.error(e); }
};

window.autoGenerateLessons = async function() {
    window.showConfirmModal('توليد الخطة العلاجية تلقائياً؟<br><small>سيتم حذف الدروس الحالية وتوليد قائمة جديدة بناءً على نتيجة التشخيص.</small>', async function() {
        try {
            const { data: diagTests } = await window.supabase.from('student_tests').select('*').eq('studentId', currentStudentId).eq('type', 'diagnostic').eq('status', 'completed');
            if (!diagTests || diagTests.length === 0) return window.showError('يجب إكمال وتصحيح الاختبار التشخيصي أولاً.'); 
            
            const compDiag = diagTests[0];
            const teacherId = window.getCurrentUser().id;
            ...
            if(newLessons.length === 0) { window.showInfoModal('الخطة العلاجية', 'الطالب متفوق! جميع المهارات محققة.'); return; }
            ...
            window.showSuccess(`تم إسناد ${newLessons.length} درس و ${newAssignments.length} واجب.`);
            window.loadLessonsTab(); window.loadAssignmentsTab(); window.calculateAndSetStudentProgress();
        } catch(e) { console.error(e); window.showError('خطأ أثناء التوليد'); }
    });
};

window.saveAndReindexLessons = async function(myList) {
    myList.forEach((l, i) => l.orderIndex = i);
    try {
        await window.supabase.from('student_lessons').delete().eq('studentId', currentStudentId);
        if (myList.length > 0) await window.supabase.from('student_lessons').insert(myList);
        window.loadLessonsTab();
    } catch(e) { console.error(e); }
};

window.moveLesson = async function(lessonId, direction) {
    try {
        const { data: myLessons } = await window.supabase.from('student_lessons').select('*').eq('studentId', currentStudentId).order('orderIndex', { ascending: true });
        if(!myLessons) return;
        ...
        await window.saveAndReindexLessons(myLessons);
    } catch(e) { console.error(e); }
};

window.accelerateLesson = function(id) {
    window.showConfirmModal('تسريع هذا الدرس؟<br><small>سيتم اعتباره منجزاً.</small>', async function() {
        await window.supabase.from('student_lessons').update({ status: 'accelerated', completedDate: new Date().toISOString() }).eq('id', id);
        window.loadLessonsTab(); window.calculateAndSetStudentProgress(); window.showSuccess('تم التسريع بنجاح');
    });
};

window.resetLesson = function(id) {
    window.showConfirmModal('إعادة فتح الدرس؟<br><small>ستُمسح النتيجة ليعيد الطالب الحل.</small>', async function() {
        ...
        window.loadLessonsTab(); window.calculateAndSetStudentProgress(); window.showSuccess('تمت إعادة فتح الدرس');
    });
};

window.deleteLesson = function(id) {
    window.showConfirmModal('حذف هذا الدرس؟', async function() {
        await window.supabase.from('student_lessons').delete().eq('id', id);
        window.loadLessonsTab(); window.calculateAndSetStudentProgress(); window.showSuccess('تم الحذف');
    });
};

window.showAssignLibraryLessonModal = async function() {
    const select = document.getElementById('libraryLessonSelect');
    if (!select) return;
    const { data: allLessons } = await window.supabase.from('lessons').select('*').eq('teacherId', window.getCurrentUser().id);
...
window.assignLibraryLesson = async function() {
    const select = document.getElementById('libraryLessonSelect');
    const lessonId = select.value;
    if (!lessonId) return window.showError('يرجى اختيار درس');
    try {
        const { data: targetLesson } = await window.supabase.from('lessons').select('*').eq('id', lessonId).single();
        if (!targetLesson) return window.showError('الدرس غير موجود');

        const { data: myLessons } = await window.supabase.from('student_lessons').select('*').eq('studentId', currentStudentId).order('orderIndex', { ascending: true });
        let list = myLessons || [];
        let strugglingIdx = list.findIndex(l => l.status === 'struggling');
        let insertIndex = list.length; let rescueId = null;

        if (strugglingIdx !== -1) { insertIndex = strugglingIdx + 1; rescueId = list[strugglingIdx].id; }

        const newStudentLesson = { id: Date.now(), studentId: currentStudentId, title: targetLesson.title, objective: targetLesson.linkedInstructionalGoal || 'درس إضافي', originalLessonId: targetLesson.id, status: 'pending', isAdditional: true, rescueLessonId: rescueId, assignedDate: new Date().toISOString(), orderIndex: 0 };
        list.splice(insertIndex, 0, newStudentLesson);
        await window.saveAndReindexLessons(list);
        window.closeModal('assignLibraryLessonModal'); window.showSuccess('تم إسناد الدرس الإضافي');
    } catch(e) { console.error(e); }
};
...
// 🔥 7. التبويب: الواجبات 🔥
// =========================================================
window.loadAssignmentsTab = async function() {
    try {
        ...
        if (!list || list.length === 0) { container.innerHTML = `<div class="empty-state"><h3>لا توجد واجبات حالياً</h3><button class="btn btn-primary" onclick="window.showAssignHomeworkModal()"><i class="fas fa-plus-circle"></i> إسناد واجب جديد</button></div>`; return; }
        
        container.innerHTML = list.map(a => `<div class="content-card"><div style="display:flex; justify-content:space-between;"><h4 style="margin:0;">${a.title}</h4><span class="badge ${a.status === 'completed' ? 'badge-success' : 'badge-primary'}">${a.status === 'completed' ? 'مكتمل' : 'جديد'}</span></div><div class="content-meta" style="margin-top:10px;"><span>📅 التسليم: ${a.dueDate || 'مفتوح'}</span><span>تاريخ الإسناد: ${new Date(a.assignedDate).toLocaleDateString('ar-SA')}</span></div><div style="margin-top:15px; border-top:1px solid #eee; padding-top:10px;">${a.status === 'completed' ? `<button class="btn btn-sm btn-outline-success" onclick="window.openReviewModal(${a.id}, 'assignment')">🔍 مراجعة الحل</button>` : '<span class="text-muted text-sm">بانتظار الحل...</span>'}<button class="btn btn-sm btn-outline-danger" style="float:left;" onclick="window.deleteAssignment(${a.id})">حذف</button></div></div>`).join('');
    } catch(e) { console.error(e); }
};

window.showAssignHomeworkModal = async function() { 
    const select = document.getElementById('homeworkSelect');
    if (!select) { injectHomeworkModal(); setTimeout(window.showAssignHomeworkModal, 50); return; }
    try {
        const { data: allAssignments } = await window.supabase.from('assignments').select('*').eq('teacherId', window.getCurrentUser().id);
...
window.assignHomework = async function() { 
    const select = document.getElementById('homeworkSelect'); 
    if(!select || !select.value) return window.showError('الرجاء اختيار واجب'); 
    try {
        ...
        window.closeModal('assignHomeworkModal'); window.loadAssignmentsTab(); window.showSuccess('تم إسناد الواجب بنجاح'); 
    } catch(e) { console.error(e); }
};

window.deleteAssignment = function(id) { 
    window.showConfirmModal('هل أنت متأكد من حذف هذا الواجب؟', async function() {
        await window.supabase.from('student_assignments').delete().eq('id', id);
        window.loadAssignmentsTab(); window.showSuccess('تم الحذف بنجاح');
    });
};

// =========================================================
// 🔥 8. التبويب: سجل التقدم والأحداث الإدارية 🔥
// =========================================================
window.loadProgressTab = async function() {
    injectAdminEventModal();
    const container = document.getElementById('section-progress');
    container.innerHTML = `<div class="content-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; flex-wrap:wrap; gap:10px;"><div><h2 style="margin:0;">سجل المتابعة اليومي</h2><span id="progressBalanceBadge" class="badge badge-success" style="font-size:1.1rem; padding:8px 15px; border-radius:8px;">الرصيد: جاري الحساب...</span></div><div class="no-print" style="display:flex; gap:10px;"><button class="btn btn-secondary" onclick="window.printProgressLog()" style="background: #475569;"><i class="fas fa-print"></i> طباعة السجل</button><button class="btn btn-primary" onclick="window.openAdminEventModal()"><i class="fas fa-plus-circle"></i> تسجيل حدث</button></div></div><div class="table-responsive" id="printableProgressArea" style="overflow-x: auto; background: white; padding: 15px; border-radius: 10px; border: 1px solid #eee; box-shadow: 0 4px 10px rgba(0,0,0,0.03);"><table class="word-table" style="min-width: 850px; width: 100%;"><thead><tr><th style="width: 30%;">اسم الدرس (الهدف)</th><th style="width: 15%;">حالة الدرس</th><th style="width: 15%;">حالة الطالب</th><th style="width: 15%;">نوع الحصة</th><th style="width: 15%;">التاريخ</th><th style="width: 10%;" class="no-print">إجراءات</th></tr></thead><tbody id="progressTableBody"><tr><td colspan="6" class="text-center p-4">جاري التحميل من السحابة...</td></tr></tbody></table></div>`;
    ...
    try {
        ...
        let myList = lessonsRes.data || []; let adminEvents = eventsRes.data || []; let teacherSchedule = scheduleRes.data || [];
        let holidays = JSON.parse(localStorage.getItem('academicCalendar') || '[]') || [];

        const tbody = document.getElementById('progressTableBody'); const badge = document.getElementById('progressBalanceBadge');
        if (myList.length === 0) { tbody.innerHTML = '<tr><td colspan="6" class="text-center p-4 text-muted">لم تبدأ الخطة بعد.</td></tr>'; badge.textContent = 'الرصيد: 0 حصة'; badge.className = 'badge badge-secondary'; return; }
        ...
        let myEvents = await window.syncMissingDaysToArchive(myList, adminEvents, teacherSchedule, planStartDate, holidays);

        let rawLogs = [];
        myList.forEach(l => { if (l.historyLog && Array.isArray(l.historyLog)) { l.historyLog.forEach(log => { if(log && log.date) { rawLogs.push({ dateObj: new Date(log.date), dateStr: new Date(log.date).toDateString(), type: 'lesson', status: log.status, title: l.title, lessonId: l.id, cachedType: log.cachedSessionType || null }); } }); } });
        myEvents.forEach(e => { if(e && e.date) { rawLogs.push({ dateObj: new Date(e.date), dateStr: new Date(e.date).toDateString(), type: e.type === 'auto-absence' ? 'auto-absence' : 'event', status: e.type, title: e.title || (e.type === 'auto-absence' ? 'درس غير محدد' : 'حدث إداري'), id: e.id, note: e.note }); } });

        let finalTimeline = []; let balance = 0; rawLogs.sort((a, b) => a.dateObj - b.dateObj);
        rawLogs.forEach(log => {
            if (log.status === 'started' || log.status === 'extension') { const hasFinalStateToday = rawLogs.some(l => l.dateStr === log.dateStr && l.lessonId === log.lessonId && ['completed', 'accelerated', 'passed_by_alternative', 'struggling', 'returned', 'pending_review'].includes(l.status)); if (hasFinalStateToday) return; }
        ...
        let rowsHtml = finalTimeline.map(item => {
            let actionsHtml = '-'; 
            if (item.actions) { actionsHtml = `<button class="btn-icon text-danger no-print" onclick="window.deleteAdminEvent(${item.actions})"><i class="fas fa-trash"></i></button>`; if (item.rowClass !== 'bg-danger-light') { actionsHtml = `<button class="btn-icon text-primary no-print" onclick="window.editAdminEvent(${item.actions})"><i class="fas fa-edit"></i></button> ` + actionsHtml; } }
...
window.syncMissingDaysToArchive = async function(myList, myEvents, teacherSchedule, planStartDate, holidays) {
    ...
    for (let d = new Date(planStartDate); d < today; d.setDate(d.getDate() + 1)) {
        ...
        const isHoliday = holidays.some(h => { if(!h.startDate || !h.endDate) return false; const start = new Date(h.startDate); const end = new Date(h.endDate); start.setHours(0, 0, 0, 0); end.setHours(23, 59, 59, 999); const checkDate = new Date(d); checkDate.setHours(12, 0, 0, 0); return checkDate >= start && checkDate <= end; }); 
        ...
        if (isScheduledDay) { newEvents.push({ id: Date.now() + Math.floor(Math.random()*10000), studentId: currentStudentId, date: new Date(d).toISOString(), type: 'auto-absence', title: lessonTitleForAbsence, note: `غياب تلقائي عن درس: ${lessonTitleForAbsence}` }); }
    }
    if (newEvents.length > 0) { await window.supabase.from('student_events').insert(newEvents); return [...myEvents, ...newEvents]; } 
    return myEvents;
};

window.openAdminEventModal = function() { document.getElementById('editingEventId').value = ''; document.getElementById('manualEventDate').valueAsDate = new Date(); document.getElementById('manualEventType').value = 'excused'; document.getElementById('manualEventNote').value = ''; document.getElementById('adminEventModal').classList.add('show'); };
window.closeAdminEventModal = function() { window.closeModal('adminEventModal'); };

window.editAdminEvent = async function(id) { 
    try {
        const { data: event } = await window.supabase.from('student_events').select('*').eq('id', id).single();
        if (!event) return; 
        document.getElementById('editingEventId').value = id; document.getElementById('manualEventType').value = event.type; document.getElementById('manualEventDate').value = event.date.split('T')[0]; document.getElementById('manualEventNote').value = event.note || ''; document.getElementById('adminEventModal').classList.add('show');
    } catch(e) { console.error(e); } 
};

window.saveAdminEvent = async function() { 
    const id = document.getElementById('editingEventId').value; const type = document.getElementById('manualEventType').value; const dateInput = document.getElementById('manualEventDate').value; const note = document.getElementById('manualEventNote').value; 
    if (!dateInput) return window.showError('يرجى اختيار التاريخ'); 
    ...
    try {
        ...
        window.closeAdminEventModal(); window.loadProgressTab(); window.showSuccess('تم حفظ الحدث بنجاح');
    } catch(e) { console.error(e); }
};

window.deleteAdminEvent = async function(id) { 
    window.showConfirmModal('هل أنت متأكد من حذف هذا السجل؟', async function() {
        await window.supabase.from('student_events').delete().eq('id', id);
        window.loadProgressTab(); window.showSuccess('تم الحذف بنجاح');
    });
};

window.printProgressLog = function() {
    const area = document.getElementById('printableProgressArea');
    if (!currentStudent || !area) return window.showError('البيانات غير جاهزة'); 
    const tableContent = area.outerHTML; const today = new Date().toLocaleDateString('ar-SA'); const printWindow = window.open('', '_blank');
    printWindow.document.write(`<html dir="rtl" lang="ar"><head><title>سجل متابعة - ${currentStudent.name}</title><style>@import url('https://fonts.googleapis.com/css2?family=Tajawal:wght@400;700&display=swap');body{font-family:'Tajawal',serif;padding:40px;color:#333;}.print-header{display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid #000;padding-bottom:10px;margin-bottom:20px;}.header-side{width:30%;font-size:13px;line-height:1.6;}.header-mid{width:40%;text-align:center;}.header-mid h2{margin:0;font-size:22px;}.student-info-box{display:grid;grid-template-columns:1fr 1fr;gap:10px;background:#f9f9f9;padding:15px;border:1px solid #000;margin-bottom:20px;border-radius:5px;}.student-info-box div{font-size:14px;}.word-table{width:100%;border-collapse:collapse;border:2px solid #000;}.word-table th,.word-table td{border:1px solid #000;padding:10px;text-align:center;font-size:13px;}.word-table th{background-color:#eee !important;font-weight:bold;-webkit-print-color-adjust:exact;}.bg-success-light{background-color:#e8f5e9 !important;-webkit-print-color-adjust:exact;}.bg-danger-light{background-color:#ffebee !important;-webkit-print-color-adjust:exact;}.bg-warning-light{background-color:#fff3e0 !important;-webkit-print-color-adjust:exact;}.bg-info-light{background-color:#e3f2fd !important;-webkit-print-color-adjust:exact;}.no-print{display:none !important;}.footer-signatures{margin-top:50px;display:flex;justify-content:space-between;text-align:center;font-weight:bold;}.footer-signatures div{width:30%;border-top:1px solid #000;padding-top:10px;}</style></head><body><div class="print-header"><div class="header-side">المملكة العربية السعودية<br>برنامج صعوبات التعلم<br>نظام ميسر التعلم</div><div class="header-mid"><h2>سجل المتابعة اليومي</h2><p>تقرير التقدم الدراسي</p></div><div class="header-side" style="text-align: left;">التاريخ: ${today}</div></div><div class="student-info-box"><div><strong>اسم الطالب:</strong> ${currentStudent.name}</div><div><strong>الصف الدراسي:</strong> ${currentStudent.grade}</div><div><strong>المادة:</strong> ${currentStudent.subject}</div><div><strong>حالة الخطة:</strong> مستمرة</div></div>${tableContent}<div class="footer-signatures"><div>توقيع معلم صعوبات التعلم</div><div>توقيع مدير المدرسة</div></div><script>window.onload=function(){window.print();window.close();}<\/script></body></html>`);
    printWindow.document.close();
};

// =========================================================
// 🔥 9. نافذة المراجعة والتصحيح التفاعلي (مراجعة الإجابات) 🔥
// =========================================================
        ...
        (q.paragraphs || []).forEach((p, pIdx) => {
            ...
            if(studentImg) {
                ...
                <div class="correction-toolbar mb-3" style="display:flex; justify-content:center; gap:10px; flex-wrap:wrap; background:#f8f9fa; padding:10px; border-radius:8px;">
                    <button type="button" class="btn btn-sm btn-outline-danger" id="tc-pen-${q.id}-${pIdx}" onclick="window.setCorrectionMode('${q.id}', '${pIdx}', 'pen')"><i class="fas fa-pen"></i> قلم أحمر</button>
                    <button type="button" class="btn btn-sm btn-outline-success" id="tc-stamp-c-${q.id}-${pIdx}" onclick="window.setCorrectionMode('${q.id}', '${pIdx}', 'stamp-correct')">✔️ ختم (صح)</button>
                    <button type="button" class="btn btn-sm btn-outline-danger" id="tc-stamp-w-${q.id}-${pIdx}" onclick="window.setCorrectionMode('${q.id}', '${pIdx}', 'stamp-wrong')">❌ ختم (خطأ)</button>
                    <button type="button" class="btn btn-sm btn-secondary" onclick="window.clearCorrection('${q.id}', '${pIdx}')"><i class="fas fa-undo"></i> تراجع/مسح</button>
                </div>
        ...
        if (q.type === 'manual-reading') {
            ...
            q.paragraphs.forEach((p, pIdx) => {
                ...
                let wordsHtml = words.map((w, wIdx) => {
                    ...
                    return `<span class="reading-word-eval ${wClass}" onclick="window.toggleReadingWord(this, '${q.id}', '${wKey}')" data-state="${wEval}">${w}${icon}<input type="hidden" name="eval_${q.id}_${wKey}" value="${wEval}"></span>`;
                }).join(' ');
            ...
            q.paragraphs.forEach((p, pIdx) => {
                ...
                let btnCorrect = `<button type="button" class="btn btn-sm ${evalState === 'correct' ? 'btn-success' : 'btn-outline-success'}" onclick="window.setEvalState(this, '${q.id}', '${pKey}', 'correct')">✔️ صحيح</button>`;
                let btnWrong = `<button type="button" class="btn btn-sm ${evalState === 'wrong' ? 'btn-danger' : 'btn-outline-danger'}" onclick="window.setEvalState(this, '${q.id}', '${pKey}', 'wrong')">❌ خاطئ</button>`;
...
function initTeacherCorrectionCanvas(qId, pIdx, originalImgBase64, existingCorrection) {
    ...
    img.onload = () => {
        ...
        tcContexts[key] = { ctx: ctx, canvas: canvas, mode: 'pen', baseImg: img }; 
        
        ctx.strokeStyle = '#dc3545'; ctx.lineWidth = 4; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
        window.setCorrectionMode(qId, pIdx, 'pen');
...
window.openReviewModal = async function(targetId, type = 'assignment') {
    ...
    } catch(e) { console.error(e); window.showError('خطأ في جلب بيانات المراجعة'); }
};

window.saveTestReview = async function() {
    ...
    try {
        ...
        if (!record) return window.showError('السجل غير موجود');
        ...
        if (type === 'lesson') {
            ...
            if (record.score >= passScore) {
                ...
                window.showSuccess('تم حفظ التصحيح. الطالب اجتاز المحك واكتمل الدرس بنجاح!');
            } else {
                updateData.status = 'returned'; 
                window.showError(`تم حفظ التقييم. نتيجة الطالب (${record.score}%) لم تحقق المحك (${passScore}%). أُعيد الدرس للطالب.`);
            }
        } else {
            updateData.status = 'completed'; window.showSuccess('تم حفظ التصحيح واعتماد الدرجة بنجاح.');
        }

        await window.supabase.from(tableName).update(updateData).eq('id', id);
        window.closeModal('reviewTestModal');
        
        if (type === 'assignment') window.loadAssignmentsTab(); 
        else if (type === 'test') window.loadDiagnosticTab(); 
        else if (type === 'lesson') { window.loadLessonsTab(); window.calculateAndSetStudentProgress(); }
    } catch(e) { console.error(e); window.showError('حدث خطأ أثناء الحفظ'); }
};

window.returnTestForResubmission = async function() {
    const id = parseInt(document.getElementById('reviewAssignmentId').value);
    const type = document.getElementById('reviewAssignmentId').getAttribute('data-type');
    window.showConfirmModal('إعادة الاختبار/الدرس للطالب لتعديل الإجابات؟', async function() {
        let tableName = type === 'test' ? 'student_tests' : (type === 'lesson' ? 'student_lessons' : 'student_assignments');
        await window.supabase.from(tableName).update({ status: 'returned' }).eq('id', id);
        window.closeModal('reviewTestModal');
        if (type === 'assignment') window.loadAssignmentsTab();
        else if (type === 'test') window.loadDiagnosticTab();
        else if (type === 'lesson') window.loadLessonsTab();
        window.showSuccess('تمت الإعادة للطالب بنجاح');
    });
};
