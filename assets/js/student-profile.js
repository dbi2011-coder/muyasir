...
// الوصف: إدارة ملف الطالب + التقييم الجزئي + الخطة بدون نسب + سجل التقدم الكامل
// ============================================
                ...
                if (obj) {
                    if (percentage >= criterion) {
                        ...
                        if (!needsObjects.find(o => o.id == obj.id)) {
                            ...
                            // 🔥 تم حذف النسبة المئوية لتظهر النقطة بشكل نظيف ورسمي 🔥
    ...
    const cardsHtml = list.map(a => `
        <div class="content-card">
            ...
            <div style="margin-top:15px; border-top:1px solid #eee; padding-top:10px;">
                ${a.status === 'completed' ? `<button class="btn btn-sm btn-outline-success" onclick="openReviewModal(${a.id}, 'assignment')">🔍 مراجعة الحل</button>` : '<span class="text-muted text-sm">بانتظار الحل...</span>'}
...
// =========================================================
// 🔥 9. قسم المراجعة والتصحيح الذكي واستخراج الإجابات 🔥
// =========================================================
...
function saveTestReview() {
    ...
    if(questions && questions.length > 0) {
        questions.forEach(q => {
            ...
            if(ansIdx === -1) {
                collection[idx].answers.push({ questionId: q.id, answer: null });
                ansIdx = collection[idx].answers.length - 1;
            }
            
            collection[idx].answers[ansIdx].score = newScore;
            collection[idx].answers[ansIdx].teacherNote = noteInp ? noteInp.value : '';
            
            if (!collection[idx].answers[ansIdx].evaluations) {
                collection[idx].answers[ansIdx].evaluations = {};
            }
            ...
            evalInputs.forEach(inp => {
                ...
                collection[idx].answers[ansIdx].evaluations[pKey] = inp.value;
            });
