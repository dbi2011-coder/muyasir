...
async function openTestMode(assignmentId) {
    try {
        const { data: assignment, error: aError } = await window.supabase.from('student_tests').select('*').eq('id', assignmentId).single();
        if (aError || !assignment) return window.showError('لم يتم العثور على بيانات الاختبار');
        
        const { data: test, error: tError } = await window.supabase.from('tests').select('*').eq('id', assignment.testId).single();
        if (tError || !test) return window.showError('نموذج الاختبار الأصلي غير موجود');

        currentAssignment = assignment;
        currentTest = test;

        if (currentAssignment.status === 'completed') {
            window.showInfoModal('وضع المراجعة', 'أنت الآن في وضع المراجعة.<br>لا يمكنك تعديل الإجابات، يمكنك فقط الاطلاع على الحلول وملاحظات المعلم وتقييمه.');
        } else if (currentAssignment.status === 'returned') {
            window.showInfoModal('تعديل الإجابات', 'أعاد المعلم هذا الاختبار إليك.<br>يمكنك الآن مراجعة الأخطاء، تعديل إجاباتك، وتسليمها مرة أخرى.');
        }
...
async function closeTestMode() {
    if (currentAssignment && currentAssignment.status !== 'completed') {
        saveCurrentCanvas();
        try {
            await window.supabase.from('student_tests')
...
function renderAllQuestions() {
    ...
    currentTest.questions.forEach((q, index) => {
        ...
        if (q.type.includes('mcq') || q.type === 'multiple-choice') {
            ...
            choices.forEach((choice, i) => {
                if (isReadOnly) {
                    ...
                    qHtml += `<div style="padding:15px; border:2px solid ${border}; border-radius:10px; margin-bottom:10px; background:${bg}; display:flex; align-items:center; justify-content:space-between; font-size:1.1rem; font-weight:bold;">
                                <div style="display:flex; align-items:center; gap:10px;"><span>${icon}</span> <span>${choice}</span></div>
            ...
            let sAns = ansValue;
            let cAns = q.data && q.data.correctValue !== undefined ? q.data.correctValue : null;

            ['true', 'false'].forEach((val) => {
                const label = val === 'true' ? 'صواب' : 'خطأ';
                if (isReadOnly) {
                    let isStudent = (String(sAns) === val);
                    let isCorrect = (String(cAns) === val);
                    let bg = isCorrect ? '#d4edda' : (isStudent ? '#f8d7da' : '#fff');
                    let border = isCorrect ? '#c3e6cb' : (isStudent ? '#f5c6cb' : '#eee');
                    let icon = isCorrect ? '✅' : (isStudent ? '❌' : '⚪');
                    
                    qHtml += `<div style="padding:15px; border:2px solid ${border}; border-radius:10px; margin-bottom:10px; background:${bg}; display:flex; align-items:center; justify-content:space-between; font-size:1.1rem; font-weight:bold;">
                                <div style="display:flex; align-items:center; gap:10px;"><span>${icon}</span> <span>${label}</span></div>
                              </div>`;
                } else {
        ...
        dropZones.forEach(zone => {
            ...
            if (text && !zone.dataset.sourceId) {
                ...
                if (match) {
                    ...
                    zone.dataset.sourceId = match.id;
...
function showQuestion(index) {
    ...
    if(card) {
        ...
        if(currentAssignment.status !== 'completed') {
            ...
            if (q.type && (q.type.includes('spelling') || q.type === 'missing-char')) {
...
async function toggleRecording(btn, qId, pIdx) {
    if (!activeRecordingId) {
        try {
            ...
            console.error(err);
            window.showError('عذراً، لم نتمكن من الوصول للميكروفون. يرجى التأكد من صلاحيات المتصفح.');
        }
    } else {
...
async function saveTestProgress(submit = false, isExiting = false) {
    ...
    if (submit) {
        ...
        currentTest.questions.forEach((q, i) => {
            ...
            const ansObj = userAnswers.find(a => a.questionId == q.id);
            ...
            if(!correct) {
                ...
                else if(currentTest.objectivesLinked) failedObjs.push(currentTest.linkedObjectiveId);
            }
...
function drop(ev) {
    ...
    if(dropZone) {
        ...
        const qIdx = dropZone.dataset.qid;
        const pIdx = dropZone.dataset.pid;
